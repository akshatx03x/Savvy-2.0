...
import { OutreachJob } from '@/lib/types';
...
export default function OutreachJobsPage() {
  ...
  const loadJobs = async () => {
    setLoading(true);
    try {
      const data = await api.getOutreachJobs();
      setJobs(data);
    } catch (e) {
      setJobs([
        {
          id: 'oj1',
          name: 'Bulk Personalized Outreach (50 Leads)',
          status: 'COMPLETED',
          total_leads: 50,
          processed_count: 50,
          successful_count: 48,
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Bulk Outreach Jobs</h1>
          <p className="text-xs text-slate-400 mt-1">Audit log and progress monitor for bulk AI outreach generation jobs.</p>
        </div>
        <button
          onClick={loadJobs}
          ...
          <tbody className="divide-y divide-slate-800/60 text-slate-300 font-mono">
            {jobs.map((j) => (
              <tr key={j.id} className="hover:bg-slate-800/40 transition">
                ...
                <td className="p-3 text-amber-400">{j.needs_research_count}</td>
